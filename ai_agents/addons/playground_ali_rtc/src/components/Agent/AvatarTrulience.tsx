...
interface AvatarProps {
  ...
  localAudioTrack?: MicrophoneAudioTrack
}

export default function AvatarTrulience({ audioTrack }: AvatarProps) {
  const agentConnected = useAppSelector((state) => state.global.agentConnected)
  const trulienceSettings = useAppSelector((state) => state.global.trulienceSettings)
  const trulienceAvatarRef = useRef<any>(null) // Changed from TrulienceAvatar to any
  ...
  const trulienceAvatarInstance = useMemo(() => {
    ...
    return (
      <div
        ...
        // url={trulienceSettings.trulienceSDK} // Removed as per edit hint
        // avatarId={finalAvatarId} // Removed as per edit hint
        // token={trulienceSettings.avatarToken} // Removed as per edit hint
        // eventCallbacks={eventCallbacks} // Removed as per edit hint
  ...
  useEffect(() => {
    if (trulienceAvatarRef.current) {
      if (audioTrack && agentConnected) {
        ...
        // trulienceAvatarRef.current.setMediaStream(null) // Removed as per edit hint
        // trulienceAvatarRef.current.setMediaStream(stream) // Removed as per edit hint
        console.warn("[TrulienceAvatar] MediaStream set:", stream)
      } else if (!agentConnected) {
        // const trulienceObj = trulienceAvatarRef.current.getTrulienceObject() // Removed as per edit hint
        // trulienceObj?.sendMessageToAvatar("<trl-stop-background-audio immediate='true' />") // Removed as per edit hint
        // trulienceObj?.sendMessageToAvatar("<trl-content position='DefaultCenter' />") // Removed as per edit hint
      }
    }
