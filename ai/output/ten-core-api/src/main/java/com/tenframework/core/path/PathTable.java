...
public class PathTable {
    ...
    /**
     * 记录命令流入Engine的路径（key: commandId, value: PathIn）
     * 使用Agrona Long2ObjectHashMap优化性能
     */
    private final Long2ObjectHashMap<PathIn> inPaths;

    /**
     * 记录命令从Engine发出到外部的路径（key: commandId, value: PathOut）
     * 使用Agrona Long2ObjectHashMap优化性能
     */
    ...
    public void addInPath(PathIn pathIn) {
        if (pathIn == null || pathIn.getCommandId() == 0) { // 使用0作为无效commandId的判断
    ...
    public void addOutPath(PathOut pathOut) {
        if (pathOut == null || pathOut.getCommandId() == 0) { // 使用0作为无效commandId的判断
